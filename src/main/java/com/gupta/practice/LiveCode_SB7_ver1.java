...
public class LiveCode_SB7_ver1 {
    ...
    // Lazy initialisation — instructions are expensive to fetch (hits a remote
    // recipe database). We defer the load until someone actually asks for them.
    // null means "not fetched yet" — not "doesn't exist".
    static class CoffeeStation {
        private Coffee signature = null; // null at startup — load deferred, not absent

        Coffee getSignature() {
            if (signature == null) {                    // first request — load now
                signature = loadFromRecipeDatabase();   // expensive: DB call, 200ms
            }
            return signature;                          // every request after — cached
        }

        private Coffee loadFromRecipeDatabase() {
            return new Coffee("Espresso", "Ethiopian", "Grind fine. 30s pre-infusion. 90°C.");
        }

        // Explicit GC hint — long-lived object holding a large reference.
        // On Java 1.4 with large heaps, null told the GC: I am done with this.
        // Modern JVMs detect reachability automatically — but the pattern still
        // appears in Android and large-heap systems where memory pressure is explicit.
        void releaseForGC() {
            signature = null;  // null as a pointer release, not an absence of value
        }
    }
    ...
    static void character() {
        ...
        // Null 2 — lazy initialisation: instructions exist but aren't loaded yet.
        // The station exists. The recipe database has the data.
        // We just haven't paid the 200ms cost of fetching it yet.
        CoffeeStation station = new CoffeeStation();
        Coffee lazy = station.getSignature();   // fetched on first access
        station.releaseForGC();                 // released back to GC after use
        Order alice = new Order("Alice", new Coffee("Espresso", "Ethiopian", null));
        //                                                                     ↑
        //                                            null = instructions not loaded yet

        // Null 3 — GC pointer release: the session is over, release the reference.
        // This tells the JVM: I no longer need this object.
        // The GC can reclaim the memory on its next cycle.
        Order session = new Order("Session", null);  // coffee reference released at shutdown

        System.out.println("Jonathan's coffee        : " + jonathan.coffee);
        // null — valid state. He's still deciding. Three minutes minimum.

        System.out.println("Alice's instructions     : " + alice.coffee.brewingInstructions);
        // null — lazy init. Not missing. Not broken. Just not loaded yet.
        // Loaded on first access. null means: nobody has asked for it yet.

        System.out.println("Session coffee (closed)  : " + session.coffee);
        // null — GC pointer release. The session ended. The reference was released.
        // The JVM's pointer to the Coffee object is now null.
        // The GC sees no live reference — it can reclaim the memory.

        System.out.println("\nStation signature (lazy) : " + lazy.brewingInstructions);
        // Loaded on demand. Before getSignature() was called — null.
        // After — fully populated. The empty cup, filled on first order.

        // Three nulls. Three intents. One value.
        // Jonathan's null means "not chosen yet."
        // Alice's null means "not fetched yet."
