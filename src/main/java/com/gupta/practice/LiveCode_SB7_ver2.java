...
public class LiveCode_SB7_ver2 {
    ...
    static class Coffee {
        String name;
        String origin;
        String brewingInstructions;   // null until loaded — lazy, not missing
    ...
    // Lazy initialisation — the brewing instructions map is expensive to load.
    // Priya's shop has 47 menu items. Each has allergen data, roast profiles,
    // and step-by-step brewing instructions pulled from a recipe service.
    // Loading all of them at startup costs 200ms and hits the database.
    // Nobody should pay that cost before a single customer has ordered.
    //
    // So the map starts null. It is loaded once, on the first request.
    // Every request after is served from the cache.
    // null here means "not fetched yet" — not "doesn't exist".
    static class BrewingGuide {

        // null at startup — not loaded yet, not absent
        private Map<String, String> instructions = null;

        String getInstructions(String coffeeName) {
            if (instructions == null) {
                instructions = loadAllFromRecipeService(); // 200ms DB call — paid once
            }
            return instructions.getOrDefault(coffeeName, "No instructions on file");
        }

        private Map<String, String> loadAllFromRecipeService() {
            // Simulates a DB/recipe-service fetch for all 47 menu items
            Map<String, String> map = new HashMap<>();
            map.put("espresso", "Grind fine. 30s pre-infusion. 90°C. 27s extraction.");
            map.put("latte",    "Double shot. Steam milk to 65°C. Pour at angle.");
            map.put("newblend", "Coarse grind. 94°C. 4 min steep. Press slowly.");
            return map;
        }

        // Explicit GC hint — long-lived object holding a large Map.
        // After the morning rush, Priya's system releases the cache.
        // null tells the JVM: this reference is no longer live.
        // The GC sees no live pointer to the Map — it can reclaim the heap.
        // On modern JVMs this is rarely needed. On Java 1.4 with large heaps,
        // or on Android, it was — and still is — a deliberate memory management call.
        void releaseForGC() {
            instructions = null; // pointer cleared — GC can reclaim the Map's heap
        }
    }
    ...
    static void character() {
        ...
        // Null 1 — valid absent value: Jonathan hasn't chosen his coffee yet
        Order jonathan = new Order("Jonathan", null);

        // Null 2 — lazy initialisation: the brewing instructions Map isn't loaded yet.
        // BrewingGuide exists. The recipe service has all 47 entries.
        // We just haven't paid the 200ms cost of fetching them.
        // null in instructions means: nobody has asked for it yet.
        BrewingGuide guide = new BrewingGuide();
        System.out.println("Before first request     : instructions map = " + guide.instructions);
        // null — not loaded. Correct. Nobody has ordered yet.

        String how = guide.getInstructions("espresso"); // first request — map loaded now
        System.out.println("After first request      : " + how);
        // Map is now populated. Subsequent calls hit the cache, not the DB.

        guide.releaseForGC(); // morning rush over — release the Map back to the JVM
        System.out.println("After releaseForGC()     : instructions map = " + guide.instructions);
        // null again — pointer cleared. GC can reclaim the heap the Map was using.

        // Null 3 — GC pointer release: the session is over, release the coffee reference.
        // This is the same pattern as releaseForGC() above but on Order.
        // The JVM's pointer to the Coffee object is set to null.
        // The GC sees no live reference and can reclaim the memory.
        Order session = new Order("Session", null); // coffee pointer released at shutdown

        System.out.println("\nJonathan's coffee        : " + jonathan.coffee);
        // null — valid state. He's still deciding.

        System.out.println("Session coffee (closed)  : " + session.coffee);
        // null — GC pointer release. Not absent. Released.

        // Three nulls. Three intents. One value.
        // Jonathan's null means "not chosen yet."
        // BrewingGuide's null means "not fetched yet — wait for first request."
        // Session's null means "the JVM can reclaim this memory."
        //
